...
public class MurderBot implements SimpleBot {
    ...
    public BotMove move(GameState gameState) {
        ...
        if (runAwayMode == true && gameState.getHero().getGold() >= 2 && gameState.getHero().getLife() <= 40) {
            Vertex move = getPath(closestPub).get(0);
            logger.info("Getting beer");
            return BotUtils.directionTowards(gameState.getHero().getPos(), move.getPosition());
        } 
       ...
       // for(int i=0;i< gameState.getActivePlayers();i++)
        /*if(gameState.getActivePlayers().get(i))*/
        	if(currentState.getHero().getLife()<=30)
        	{
        		Vertex move=getPath(closestPlayer).get(0);
        		logger.info("getting the hero");
        		 return BotUtils.directionTowards(gameState.getHero().getPos(), move.getPosition());
        		
        	}
        if(currentState.getHero().getMineCount()>=2)
        {
        	Vertex move=getPath(closestPlayer).get(0);
        			 return BotUtils.directionTowards(gameState.getHero().getPos(), move.getPosition());
        			
        }
      /*  for(gameState.getPosition: position)
        {
        ...
        // Step two:  Shank someone. if( gameState.getActivePlayers())
        Vertex move = getPath(closestPlayer).get(0);
